...
It completes one read-only preflight across every active scope before the first installation change. Preflight failures leave all scopes unchanged; unknown client mappings return one updater-refresh command without exposing installer keys. Missing or outdated Node.js uses the exact one-question confirmation contract and only an already available trusted installer. A failure after mutation begins stops later writes, reports partial completion with one retry action, and never attempts automatic rollback.
