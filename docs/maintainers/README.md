...
Canonical, directly installable skills live under `skills/`. Marketing Context stores reusable project facts; Editor performs source-faithful text editing; Editor in Chief runs the explicit strategic assignment workflow and hands editing back to Editor; Update maintains source-scoped installations from the repository's `main`.
