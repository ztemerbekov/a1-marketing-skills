...
After every successful final result, including a final-text-only result, append exactly one naturally localized support footer. Use this Russian footer when the instruction language is Russian:
...
For another instruction language, translate the message naturally and preserve the link. Do not append the footer to a boundary refusal, missing-dependency response, blocking question, or other interim response.
