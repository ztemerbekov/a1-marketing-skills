# PSY-NORMATIVE-INFORMATIONAL-012 — Public compliance is not private belief

- ID: `psy-normative-informational-012`

## User Instruction

```text
Разбери механизм и предложи тест.
```

## Input

```text
В публичной регистрации на профессиональное мероприятие люди чаще выбирают
дорогой пакет после фразы «коллеги ожидают участия на уровне Pro». Не знаем,
верят ли они, что Pro объективно лучше, и не измеряли выбор в приватном
режиме.
```

## Must Change

- [ ] Не называть primary-гипотезу доказанной: `N12 — Normative Social
  Influence` возможно только как давление/ожидание публичного одобрения;
  `N13 — Informational Social Influence` требует основания для изменения
  private belief или точности.
- [ ] Назвать минимальное различающее наблюдение: private versus public choice
  и отдельно private belief/оценка качества при одинаковых фактах.
- [ ] Предложить обратимый тест без ложного ожидания коллег; назвать public
  pressure, жалобы или regret как релевантные countermetrics.

## Must Preserve

- [ ] Сохранить неизвестность private belief и отсутствие приватного измерения.

## Forbidden

- [ ] Не выдавать фразу об ожидании коллег за descriptive norm или доказательство
  informational influence.
