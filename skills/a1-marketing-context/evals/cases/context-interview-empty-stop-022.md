...
- [ ] On `Стоп`, close immediately with a short Russian response stating that no context was created because no confirmed answers were saved.
...
`First turn: Russian pre-interview notice plus one question. Stop turn: one short Russian zero-write close response with no link or section list.`
