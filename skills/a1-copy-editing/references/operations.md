...
5. Treat **Explain Edits**, **Variants**, and **Substantive Review** as output modifiers, not replacements for an editing operation.
...
Select this modifier when the user requests both a rewrite and critique, analysis, or prioritized feedback on the same draft. Perform the editing operation, then report only findings that help the user evaluate the result, understand a remaining risk, or make a decision the rewrite cannot make for them.

Do not repeat the change summary as review. Follow the combined output branch in `../SKILL.md` and omit recommendations already completed by the rewrite.
...
|---------------------|-----------|
| Useful outcome, main point, reading mode, global order, opening, or ending | [text-level.md](text-level.md) |
...
| Tone, empathy, authentic voice, examples, mechanisms, implementation, or explanatory depth | [tone-and-depth.md](tone-and-depth.md) |

Do not load every conditional reference by default. **Restructure** usually needs text and paragraph levels; **Clarify** usually needs sentence and paragraph levels; **Text Review** selects only the references needed to support its prioritized findings; other operations select only the levels evidenced by the source.
