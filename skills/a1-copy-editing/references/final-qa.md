...
- The reader can identify the source-supported useful outcome and main point early enough for the format.
- Structure, opening, and ending serve the reader job rather than a default template or unnecessary chronology.
- For an editing operation, the shared correctness principle in [canon-core.md](canon-core.md) passes: grammar, grammatical government, collocation, and wording are natural and correct.
...
- Tone preserves a credible reader relationship and meaningful voice; source-supported examples, mechanisms, implementation, tradeoffs, and limits remain when they provide necessary depth.
- The result does not become generic, manipulative, or falsely confident.

## 5. Response Integrity

- The response follows every content, order, and exception rule in the output contract in `../SKILL.md`.
- An editing result contains one to five concise explanations of changes actually made, unless the user requested edited text only.
- A **Text Review** result contains prioritized findings and recommendations without claiming changes were made.
