...
1. main point;
2. necessary context;
3. proposed or decided response;
4. timing;
5. supporting detail.

Put the recipient set's current operational question or awareness/status need first in every message, including a selected-draft rewrite. Open with the current situation and any response, decision, action, or reply the recipient set needs; preserve the draft's supported meaning, not its source order. Move history below the current state when chronology would delay the job.

## Source processing

Summarize a forward or correspondence chain into a short conclusion-bearing update. Preserve source-supported event names, dates, statuses, dependencies, and decision boundaries that matter to the recipient set. Apply the comparison and evidence rules in [`source-policy.md`](source-policy.md) when options or readiness/current-state claims are present. Compress the source trail around those details rather than replacing them with a generic summary, and summarize rather than paste the thread. When rewriting a selected draft, preserve its supported facts, decision status, and requested tone while replacing its source order with the recipient set's current operational question or awareness/status need.

## Blocks and channel

Choose optional blocks from the recipient set's likely questions rather than applying a mandatory template. Include only the blocks that help the recipient set understand the current status, decide, act, or reply:

- current state or main point;
- why it matters;
- proposed or decided response;
- alternatives or rationale when supplied and material;
- timing, dependency, or consequence;
...
A fully assembled message should let the recipient understand the current status or answer, decide, act, or acknowledge as requested without preventable follow-up. If a missing fact makes that impossible, use the blocking rule in [`source-policy.md`](source-policy.md) rather than choosing a material meaning or decision.
