...
Before starting, read [the scope and source boundary](references/scope-and-sources.md). If the request is in scope, follow [the humanization loop](references/humanization-loop.md) and load [the pattern catalog](references/pattern-catalog.md) during every run. [License and attribution](references/license-and-attribution.md) records provenance and is not a runtime instruction.
