...
`node`, `npm`, and `npx` are unavailable. Homebrew can install a Node.js release satisfying the current CLI requirement.
...
- Continuing to `npx skills` before the current Node.js requirement is verified.
